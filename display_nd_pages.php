...
<body>
	...
	<div>
		...
		<a href="javascript:void(0)" onclick="clear_nd_folder()" class='btn btn-primary' style="float:right;">Clear nai dunia Folder</a>
	</div>
	...
	<form method="POST" action="display_nd_pages.php" id="submit_form">
	...
	<script>
		...
		function clear_nd_folder() {
			$("#action").val('Clear_nd_image_folder');
		...
		function initCropper(image_name, filename) {
			...
			$.ajax({
				...
				success: function(response) {
					$("#loader").fadeOut();
					var image_url = './nd_images/' + filename;
